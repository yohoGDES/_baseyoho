<?php 
/* Example Usage:
 * rpm_add_image_size( 'product-screenshot', 300, 300, array( 'left', 'top' ) );
 * rpm_add_image_size( 'product-feature', 460, 345, array( 'center', 'top' ) );
 */

add_filter( 'intermediate_image_sizes_advanced', 'rpm_intermediate_image_sizes_advanced' );
add_filter( 'wp_generate_attachment_metadata', 'rpm_generate_attachment_metadata', 10, 2 );
 ...
 */
function rpm_add_image_size( $name, $width = 0, $height = 0, $crop = false ) {
 ...
 */
function rpm_intermediate_image_sizes_advanced( $sizes ) {
  return array();
}


function rpm_generate_attachment_metadata( $metadata, $attachment_id ) {
  ...
  if ( !preg_match('!^image/!', get_post_mime_type( $attachment )) || !file_is_displayable_image( $file ) ) return $metadata;
    ...
    foreach ( $sizes as $size => $size_data ) {
        $resized = rpm_image_make_intermediate_size( $file, $size_data['width'], $size_data['height'], $size_data['crop'] );
        if ( $resized )
 ...
 * @return bool|array False, if no image was created. Metadata array on success.
 */
function rpm_image_make_intermediate_size( $file, $width, $height, $crop = false ) {
  if ( $width || $height ) {
    $resized_file = rpm_image_resize( $file, $width, $height, $crop, null, null, 90 );
 ...
 */
function rpm_image_resize_dimensions($orig_w, $orig_h, $dest_w, $dest_h, $crop = false) {
 ...
 */
function rpm_image_resize( $file, $max_w, $max_h, $crop = false, $suffix = null, $dest_path = null, $jpeg_quality = 90 ) {
  ...
  $dims = rpm_image_resize_dimensions($orig_w, $orig_h, $max_w, $max_h, $crop);
  if ( !$dims )
